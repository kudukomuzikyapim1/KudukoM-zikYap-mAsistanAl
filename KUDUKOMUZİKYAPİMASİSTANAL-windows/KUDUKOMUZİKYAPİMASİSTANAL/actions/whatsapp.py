# actions/whatsapp.py - WhatsApp entegrasyonu (Sesli/Görüntülü Arama + Mesaj Gönderme)
"""
WhatsApp mesaj gönderme ve arama yapma - Windows Desktop/Web uyumlu.
"""
...
def _find_contact_number(contact_name: str) -> str:
    """Kayıtlı WhatsApp kişilerinden numara bulmaya çalışır"""
    contact = _find_contact(contact_name)
    if contact:
        return str(contact.get("value", ""))
    ...
    for name, data in contacts.items():
        if contact_name.lower() in name.lower() or contact_name.lower() in data.get("aliases", "").lower():
            return data.get("value", "")
    
    return ""


def call_whatsapp(recipient_name: str, phone_number: str = "", video_call: bool = False) -> str:
    """
    WhatsApp üzerinden sesli veya görüntülü arama yapar
    
    Args:
        recipient_name: Kişi adı
        phone_number: Telefon numarası (opsiyonel)
        video_call: True ise görüntülü arama, False ise sesli arama
    
    Returns:
        str: İşlem sonucu
    """
    try:
        # Telefon numarası yoksa ve kişi adı varsa, önce kişiyi bulmaya çalış
        if not phone_number and recipient_name:
            phone_number = _find_contact_number(recipient_name)
            if not phone_number:
                return f"'{recipient_name}' kişisi bulunamadı. Önce kişiyi WhatsApp kişilerinize ekleyin veya telefon numarası belirtin."
        
        # Arama başlat
        if phone_number:
            # Telefon numarasından temizleme
            ...
            if video_call:
                whatsapp_url = f"whatsapp://videocall?phone={clean_number}"
                call_type = "görüntülü"
            else:
                whatsapp_url = f"whatsapp://call?phone={clean_number}"
                call_type = "sesli"
            
            subprocess.Popen(["start", whatsapp_url], shell=True)
            return f"WhatsApp'te {recipient_name or phone_number} ile {call_type} arama başlatılıyor..."
        
        return "Arama başlatılamadı: Geçerli bir kişi veya numara bulunamadı."
        
    except Exception as e:
        return f"Arama başlatılırken hata: {str(e)}"
