"""
KUDUKOMUZİKYAPİMASİSTANAL Windows — UI v3
...
SYSTEM_NAME = "🤖 KUDUKOMUZİKYAPİMASİSTANAL"
...
class JarvisUI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("KUDUKOMUZİKYAPİMASİSTANAL")
    ...
    def _build_social_bar(self):
        ...
        name_lbl = tk.Label(
            bar, text="KUDUKOMUZİKYAPİMASİSTANAL",
            fg="#3a8a82", bg=C_BG,
            font=font_display(14), cursor="hand2",
            justify="left",
        )
        name_lbl.pack(side="left", padx=(0, 10))
        name_lbl.bind("<Button-1>", _open("https://www.instagram.com/kudukomuzikyapimasistanal/"))
        ...
        if self._icon_ig:
            ...
            ig_lbl.bind("<Button-1>", _open("https://www.instagram.com/kudukomuzikyapimasistanal/"))

        if self._icon_yt:
            yt_lbl = tk.Label(bar, image=self._icon_yt, bg=C_BG, cursor="hand2")
            yt_lbl.pack(side="left", padx=4)
            yt_lbl.bind("<Button-1>", _open("https://www.instagram.com/kudukomuzikyapimasistanal/"))
    ...
    def _toggle_pause(self):
        ...
        if self.paused:
            ...
            self.write_log("SYS: KUDUKOMUZİKYAPİMASİSTANAL duraklatıldı.")
        else:
            self.set_state("THINKING")
            self.write_log("SYS: KUDUKOMUZİKYAPİMASİSTANAL devam ediyor...")
        self._sync_sound_state()
    ...
    def _shutdown(self):
        self.sound.stop_all()
        self.write_log("SYS: KUDUKOMUZİKYAPİMASİSTANAL kapatılıyor...")
    ...
    def _on_input_submit(self, event=None):
        ...
        if not text:
            return
        if self.paused:
            self.write_log("SYS: KUDUKOMUZİKYAPİMASİSTANAL duraklatılmış durumda. Devam etmek için pause'u kapat.")
            return
        self._input_var.set("")
    ...
    def _draw(self):
        ...
        c.create_text(W//2, H-13, fill=C_DIM, font=font_body(9),
                      text="KUDUKOMUZİKYAPİMASİSTANAL · Windows Edition · Realtime Voice Core")
    ...
    def _save_api_key(self):
        ...
        if was_ready:
            ...
            self.set_state("LISTENING")
            self.write_log("SYS: KUDUKOMUZİKYAPİMASİSTANAL hazır. Dinliyorum...")
